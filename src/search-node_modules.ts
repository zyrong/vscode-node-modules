...
export default async function (uri: Uri) {
  ...
  if (!node_modulesPath) {
    const projectRootPath = await showPickWorkspaceFolder();
    node_modulesPath = join(projectRootPath, NODE_MODULES);
  }
  const filePath = await showQuickPickFile(node_modulesPath, join(basename(dirname(node_modulesPath)), NODE_MODULES));
  if (filePath) {
